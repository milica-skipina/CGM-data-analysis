...
cgm_vs_bgm.filter("PtID == 19").orderBy(["DiffInSeconds", "DateTimeBGM"]).show(50)
cgm_vs_bgm = cgm_vs_bgm.filter("DiffInSeconds < 300")\
    .withColumn("GlucoseValueDiff", abs(col("GlucoseValue") - col("GlucoseValueBGM")))
cgm_vs_bgm.show(10)
cgm_vs_bgm.agg(min("GlucoseValueDiff"), max("GlucoseValueDiff")).show()
cgm_vs_bgm.orderBy("GlucoseValueDiff", ascending=False).show(50)
cgm_vs_bgm.orderBy("GlucoseValueDiff", ascending=True).show(50)
temp = cgm_vs_bgm.groupBy("GlucoseValueDiff").count()
print("GlucoseValueDiff count")
temp.filter('count > 10000').orderBy("count",  ascending=True).show(50)
temp.filter('count < 100').orderBy("count",  ascending=False).show(10)


cgm_vs_bgm = cgm_vs_bgm.withColumn("PercentageDiff", when((col("GlucoseValueBGM") > 80),
                                                          col("GlucoseValueDiff")/col("GlucoseValueBGM")).otherwise(-1))

bad_value = cgm_vs_bgm.filter((col("PercentageDiff") > 20) |
                              ((col("PercentageDiff") == -1) & (col("GlucoseValueDiff") > 20)))

print("Bad value CGM VS BGM")
print(bad_value.count())
print("of total")
print(cgm_vs_bgm.count())
bad_value.show(20)
...
# dupes = join_df.groupBy("RecIDBGM").count().filter("count > 1")
# dupes.join(join_df, "RecIDBGM").orderBy("RecIDBGM").show(50)


# prosjecna vrijednost BGM kad je razlika izmedju CGM i BGM > 100
...
bad_value.groupBy("GlucoseSeverity").count().show(20)
